...
@pytest.fixture
def vns_tester(tester_provider):
...
@pytest.fixture
def foo_contract(vns_tester, w3):
    ...
    deploy_address = vns_tester.get_accounts()[0]
    ...
    FooContract = w3.vns.contract(abi=abi, bytecode=bytecode)
    # issue a transaction to deploy the contract.
    tx_hash = FooContract.constructor().transact({
        'from': deploy_address,
    })
    # wait for the transaction to be mined
    tx_receipt = w3.vns.waitForTransactionReceipt(tx_hash, 180)
...
def test_can_update_greeting(w3, foo_contract):
    ...
    tx_hash = foo_contract.functions.setBar(
        ...
        'from': w3.vns.accounts[1],
    })
    w3.vns.waitForTransactionReceipt(tx_hash, 180)
...
def test_updating_greeting_emits_event(w3, foo_contract):
    ...
    receipt = w3.vns.waitForTransactionReceipt(tx_hash, 180)
