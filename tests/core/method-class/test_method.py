...
def test_method_accepts_callable_for_selector():
    method = Method(
        mungers=[],
        json_rpc_method=lambda *_: 'vns_method',
        formatter_lookup_fn=''
    )
    assert method.method_selector_fn() == 'vns_method'


def test_method_selector_fn_accepts_str():
    method = Method(
        mungers=[],
        json_rpc_method='vns_method',
        formatter_lookup_fn=''
    )
    assert method.method_selector_fn() == 'vns_method'
...
def test_get_formatters_non_falsy_config_retrieval():
    ...
    assert method.get_formatters('vns_method') == 'match'
    assert method.get_formatters('vns_nonmatching') == 'nonmatch'
