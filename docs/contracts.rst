...
- The sandbox node provided by vns-tester. You can install it with ``pip install -U web3[tester]``.
...
.. code-block:: python
    ...
    Greeter = w3.vns.contract(abi=contract_interface['abi'], bytecode=contract_interface['bin'])
    ...
    w3.vns.waitForTransactionReceipt(tx_hash)
...
Instead, create contract objects using the :meth:`w3.vns.contract() <web3.vns.Bbbbbbbb.contract>`
...
.. py:class:: Contract(address)
    ...
    The address parameter can be a hex address or an ENS name, like ``mycontract.vns``.
...
.. py:class:: ConciseContract(Contract())
    ...
    .. code-block:: python
        ...
        >>> concise.withdraw(amount, transact={'from': vns.accounts[1], 'gas': 100000, ...})
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).transact(transaction=None)
    ...
    ``web3.vns.sendTransaction(transaction)`` method.  This value may not
    ...
    deployment transaction will be created using the ``web3.vns.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> deploy_txn = token_contract.constructor(web3.vns.coinbase, 12345).transact()
        >>> txn_receipt = web3.vns.getTransactionReceipt(deploy_txn)
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).estimateGas(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> token_contract.constructor(web3.vns.coinbase, 12345).estimateGas()
        12563
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).buildTransaction(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> transaction = {
        'gasPrice': w3.vns.gasPrice,
        'chainId': None
        }
        >>> contract_data = token_contract.constructor(web3.vns.coinbase, 12345).buildTransaction(transaction)
        >>> web3.vns.sendTransaction(contract_data)
...
.. py:classmethod:: Contract.events.<event name>.createFilter(fromBlock=block, toBlock=block, argument_filters={"arg1": "value"}, topics=[])
    ...
    ``topics`` optional, accepts the standard JSON-RPC topics argument.  See the JSON-RPC documentation for `vns_newFilter <https://github.com/ethereum/wiki/wiki/JSON-RPC#vns_newfilter>`_ more information on the ``topics`` parameters.
...
For example:
    ...
    .. code-block:: python
        ...
        myContract = web3.vns.contract(address=contract_address, abi=contract_abi)
...
.. py:method:: ContractFunction.transact(transaction)
    ...
    the same format as the ``web3.vns.sendTransaction(transaction)`` method.
    ...
    If a ``gas`` value is not provided, then the ``gas`` value for the
    method transaction will be created using the ``web3.vns.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> token_contract.functions.transfer(web3.vns.accounts[1], 12345).transact()
...
.. py:method:: ContractFunction.call(transaction, block_identifier='latest')
    ...
    ``vns_call`` API.  This will not create a new public transaction.
    ...
    .. code-block:: python
        ...
        21
        >>> token_contract.functions.myBalance().call({'from': web3.vns.coinbase})
        12345  # the token balance for `web3.vns.coinbase`
        >>> token_contract.functions.myBalance().call({'from': web3.vns.accounts[1]})
        54321  # the token balance for the account `web3.vns.accounts[1]`
...
For example:
    ...
    .. code-block:: python
        ...
        receipt = web3.vns.getTransactionReceipt(tx_hash)
...
.. py:method:: ContractEvents.myEvent(*args, **kwargs).processReceipt(transaction_receipt)
   ...
   .. code-block:: python
       ...
       >>> tx_receipt = w3.vns.getTransactionReceipt(tx_hash)
...
.. py:classmethod:: Contract.decode_function_input(data)
    ...
    .. code-block:: python
        ...
        >>> transaction = w3.vns.getTransaction('0x5798fbc45e3b63832abc4984b0f3574a13545f415dd672cd8540cd71f735db56')
        >>> transaction.input
...
.. testsetup::
   ...
   contract = w3.vns.contract(abi=ABI, bytecode=bytecode)
   deploy_txn = contract.constructor().transact()
   deploy_receipt = w3.vns.waitForTransactionReceipt(deploy_txn)
   address = deploy_receipt.contractAddress

.. doctest::

   >>> myContract = w3.vns.contract(address=address, abi=ABI)
...
.. doctest::
   ...
   >>> from_address = w3.vns.accounts[1]
